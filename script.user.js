...
(function () {
  ...
  function protectSensitiveInfo() {
    ...
    // Array of excluded usernames or phrases
    const exclusions = ["susanburroughsguide", "anotherusername", "yetanotherexclusion"];

    // Check if any exclusion is present
    if (exclusions.some(exclusion => document.body.textContent.includes(exclusion))) return;
  ...
  // Add a debounce delay to prevent repeated firing of the MutationObserver
  let debounceTimer;
  ...
  const observer = new MutationObserver(() => {
    ...
    debounceTimer = setTimeout(() => {
      protectSensitiveInfo();
    }, 200); // 200ms debounce delay to reduce excessive triggers
  });
